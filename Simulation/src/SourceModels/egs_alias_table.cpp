 ...
 */
const EGS_I64 loopMax = (EGS_I64)1e10;
...
void EGS_AliasTable::make() {
    ...
    for (i=0; i<np-1; i++) {
        ...
        if (low_bin < 0) {
            printf("EGS_AliasTable::make(): found a high bin, but no low bin; this is abnormal.");
            break;
        }
...
int EGS_AliasTable::initialize(EGS_Float xmin, EGS_Float xmax, EGS_Float accu,
    ...
    for (EGS_I64 loopCount=0; loopCount<=loopMax; ++loopCount) {
        if (loopCount == loopMax) {
            printf("EGS_AliasTable::initialize: Too many iterations were required! Input may be invalid, or consider increasing loopMax.");
            return 1;
        }
        int nnn = (n-1)*AT_NCHECK + n;
        xtemp = new EGS_Float[nnn];
        ftemp = new EGS_Float[nnn];
        if (!xtemp || !ftemp) printf("EGS_AliasTable::initialize: "
                                           "not enough memory!\n");
        bool is_ok = true;
        int i,j=0;
        for (i=0; i<n-1; i++) {
...
int EGS_AliasTable::sampleBin() const {
    EGS_Float r1 = Random::UniformRand();
    EGS_Float aj = r1*np;
    int j = (int) aj;
    aj -= j;
    if (aj > wi[j]) {
...
EGS_Float EGS_AliasTable::sample() const {
    EGS_Float r1 = Random::UniformRand();
    EGS_Float aj = r1*np;
    int j = (int) aj;
    aj -= j;
    ...
    EGS_Float r2 = Random::UniformRand();
    if (type == 1) {
        return x + dx*r2;
    }
    EGS_Float res;
...
EGS_SimpleAliasTable::EGS_SimpleAliasTable(int N, const EGS_Float *f) : n(0) {
    ...
    if (sum <= 0) {
        printf("Error: %s, line %d: degenerate distribution, histogram sum <= 0", __FILE__, __LINE__);
    }
    else {
        for (i=0; i<n; i++) {
            p[i] /= sum;
    ...
    while (big_list.size() > 0 && small_list.size() > 0 && loopCount++ <= loopMax) {
        ...
        int smal = small_list.back();

        // alias: fill small bin
        bins[smal] = big;              // alias small to big
        wi[big] -= (1.0 - wi[smal]);   // remove aliased portion from big bin
    ...
    if (big_list.size() > 0) {
        printf("Warning: %s, line %d: table aliasing may be incomplete", __FILE__, __LINE__);
    }
