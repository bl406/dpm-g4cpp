...
namespace SimpleSourceKernel {
	__global__ void InitTracksKernel(Track* track, int n, float ekin, int type, float lbox) {
...
void SimpleSource::InitTracks(Track* track, int n) const {
	...
	SimpleSourceKernel::InitTracksKernel << <nblocks, THREADS_PER_BLOCK >> > (track, n, fEkin, fType, fLBox);
	CudaCheckError();
}