...
void   Simulate_kernel(int i, Geom& geom, float theElectronCut, float theGammaCut, SimMaterialData& matData) {

	Track& track = h_TrackSeq.fData[i];
 ...
 //          float step =
	...
	if (track.fType == 0) {
		KeepTrackingPhoton(matData, geom, track);
		return;
	}
	//
  ...
  for (int ibatch = 0; ibatch < nbatch; ++ibatch) {
      ...
      while (1) {
		  ...
		  for(int i = 0; i < h_TrackSeq.size(); ++i)
            Simulate_kernel(i, geom, theElectronCut, theGammaCut, matData);
           h_TrackSeq.fSize = 0;
      }
...
void PerformBrem(Track& track) {
 ...
 // insert the secondary gamma track into the stack
  Track& aTrack = h_PhotonStack.push_one();
 aTrack.fType         = 0;
 aTrack.fEkin         = eGamma;
 aTrack.fMatIndx      = track.fMatIndx;
 aTrack.fPosition[0]  = track.fPosition[0];
...
void PerformAnnihilation(Track& track) {
  ...
  Track& aTrack1 = h_PhotonStack.push_one();
  aTrack1.fType         = 0;
  aTrack1.fEkin         = kEMC2;
